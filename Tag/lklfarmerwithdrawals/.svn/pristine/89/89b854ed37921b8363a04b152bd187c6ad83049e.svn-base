...
/**
 * DES/3DES加解密的辅助类 完善补充工作者 Zhixiang Liu 完善时间 2012.05.07
 * 
 * @原作者 Jianping Wang
 */
public class SecurityUtil {
	...
	public String ansiMacDES(String key, String vector, String source, boolean isHex) throws Exception {
		return mac(isHex ? hexStringToByte(source.toUpperCase()) : source.getBytes(),
				hexStringToByte(key.toUpperCase()), hexStringToByte(vector.toUpperCase()));
	}
	...
	public String ansiMac3DES(String source, String key, String vector, boolean isHex) throws Exception {
		return mac3Des(isHex ? hexStringToByte(source.toUpperCase()) : source.getBytes(),
				hexStringToByte(key.toUpperCase()), hexStringToByte(vector.toUpperCase()));
	}
	...
	public byte[] encryptECB(byte[] key, byte[] source) {
		...
		byte[] currorEncryptResult = key.length > 8 ? encrype3Des(key, cursorSourntBytes)
				: encryptDes(key, cursorSourntBytes);
		if (source.length > 8) {
	...
	public byte[] decryptECB(byte[] key, byte[] source) {
		byte[] cursorSourntBytes = new byte[8];
		System.arraycopy(source, 0, cursorSourntBytes, 0, source.length > 8 ? 8 : source.length);
		byte[] currorDecryptResult = key.length > 8 ? decrypt3Des(key, cursorSourntBytes)
				: decryptDes(key, cursorSourntBytes);
		if (source.length > 8) {
			byte[] nextSource = new byte[source.length - 8];
			System.arraycopy(source, 8, nextSource, 0, source.length - 8);
	...
	public String encryptCBC(String key, String vector, String source) throws Exception {
		return bcd2str(encryptCBC(hexStringToByte(key.toUpperCase()),
				hexStringToByte(vector != null ? vector : "0000000000000000"), hexStringToByte(source.toUpperCase())));
	}
	...
	public byte[] encryptCBC(byte[] key, byte[] vector, byte[] source) throws Exception {
		...
		byte[] currorEncryptResult = key.length > 8 ? encrype3Des(key, xorResultBytes)
				: encryptDes(key, xorResultBytes);
		if (source.length > 8) {
			byte[] nextSource = new byte[source.length - 8];
			System.arraycopy(source, 8, nextSource, 0, source.length - 8);
	...
	public String decryptCBC(String key, String vector, String source) throws Exception {
		return bcd2str(decryptCBC(hexStringToByte(key.toUpperCase()),
				hexStringToByte(vector != null ? vector : "0000000000000000"), hexStringToByte(source.toUpperCase())));
	}
