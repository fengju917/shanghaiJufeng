 ...
 * 输入交易凭证号
 */
public class InputTransBillno extends TradeBaseActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		info_lable.setText("请输入原交易凭证号");
		mEditText.setHint("请输入原交易凭证号");
		...
		nextBtn.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				if (null == transInfo || "".equals(transInfo)) { // 如果原交易凭证号为空
					DialogFactory.showTips(InputTransBillno.this, "原交易凭证号输入不能为空");
				} else if (transInfo.length() > 6) {
					DialogFactory.showTips(InputTransBillno.this, "原交易凭证号位数不对");
				} else {
					transInfo = Utility.addZeroForNum(transInfo, 6);
					transInfo = mTransaction.getDataMap().get("batchbillno").subSequence(0, 6) + transInfo;
