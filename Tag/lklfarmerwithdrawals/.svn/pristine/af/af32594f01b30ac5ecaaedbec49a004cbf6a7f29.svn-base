...
public class ScriptNotityDaoImpl implements ScriptNotityDao {
	...
	private final Logger logger = Logger.getLogger(ScriptNotityDaoImpl.class);

	private Context context;
	private DBOpenHelper openHelper;

	public ScriptNotityDaoImpl(Context context) {
	...
	public synchronized ScriptNotity getScriptNotity() {
		...
		try {
			...
			logger.error("获取scriptnotity发生异常!", e);
			e.printStackTrace();
		}
		cursor.close();
		return record;
	...
	public synchronized ScriptNotity getScriptNotityByCondition(String batchno, String systraceno) {
		ScriptNotity record = null;
		if (batchno == null || "".equals(batchno) || systraceno == null || "".equals(systraceno)) {
			return record;
		}
		SQLiteDatabase db = openHelper.getReadableDatabase();
	...
	@Override
	public synchronized ScriptNotity getScriptNotityByCondition(String transprocode, String systraceno,
			String terminalid, String acceptoridcode) {
		ScriptNotity record = null;
		SQLiteDatabase db = openHelper.getReadableDatabase();
		Cursor cursor = db.rawQuery(
				"select * from scriptnotity where transprocode = ? and systraceno = ? and terminalid = ? and acceptoridcode = ?",
	...
	public synchronized int save(ScriptNotity record) {
		...
		try {
			db.execSQL(
					"insert into scriptnotity(" + "priaccount,transprocode,transamount,systraceno,translocaltime,"
			...
			result = -1;
			logger.error("保存scriptnotity异常", e);
		}
		return result;
	}
	...
	public synchronized int update(ScriptNotity record) {
		...
		try {
			...
			result = -1;
			logger.error("更新scriptnotity异常", e);
		}
		return result;
	}
	...
	public synchronized int update(int id, String key, String value) {
		...
		SQLiteDatabase db = openHelper.getWritableDatabase();
		try {
			db.execSQL("update scriptnotity set " + key + "=? where id=?", new Object[] { value, id });
			result = 1;
			M3Utility.sync(); // 同步命令
		} catch (Exception e) {
			result = -1;
	...
	public synchronized int delete(ScriptNotity record) {
		...
		SQLiteDatabase db = openHelper.getWritableDatabase();
		try {
			db.execSQL("delete from scriptnotity where id=?", new Object[] { record.getId() });
			result = 1;
			M3Utility.sync(); // 同步命令
		} catch (Exception e) {
			result = -1;
			logger.error("删除scriptnotity异常", e);
		}
		return result;
	}
	...
	public synchronized int deleteAll() {
		...
		SQLiteDatabase db = openHelper.getWritableDatabase();
		try {
			db.execSQL("delete from scriptnotity", new Object[] {});
			result = 1;
			M3Utility.sync(); // 同步命令
		} catch (Exception e) {
			result = -1;
